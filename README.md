...
```
b15f23bda75bdb7b390b526eebd653261d013e1f9089a1d1e945c53b1d710941
da5374fb14708e9ae824451ec51269e6e74120f19c3a1658bf27aac1c39973b5
e1ea7fe1d91209e7ba23f6a3ad0d32d1d8c07bb3d22a34fb0f1cd2b2b0ceee7b
```
...
- 地址信息存储在 `addresses.json` 文件中，作为私钥和地址的映射
...
- `account.json` - 私钥列表（纯文本格式，每行一个私钥）
- `addresses.json` - 私钥和地址的映射
- `proxies.txt` - 代理列表
- `referral.txt` - 邀请码
- `index.js` - 主程序
- `start.js` - 启动脚本
...
系统会自动从私钥派生出正确的以太坊地址（以0x开头）。如果您看到地址不正确，可能是因为您手动编辑了`addresses.json`文件。请使用脚本的账号管理功能添加账号，系统会自动计算正确的地址。

### **7️⃣ 如何一次性添加多个账号？**

在账号管理功能中，选择"添加账号"选项，然后连续输入多个私钥和对应的代理地址。每输入一个账号后，系统会提示您继续输入下一个账号，或者直接按回车键结束输入。

### **8️⃣ 为什么启动机器人后，状态检查显示"机器人未在运行"？**

这可能是由于进程检测问题导致的。我们已经修复了这个问题，现在机器人启动后会等待一秒，确保进程已经启动，并且在状态检查时会显示详细的进程信息。如果您仍然遇到这个问题，可以尝试手动检查进程：

```bash
ps aux | grep "node.*start.js" | grep -v grep
```

### **9️⃣ 为什么我添加的账号没有显示在account.json文件中？**

这可能是由于文件路径问题导致的。我们已经修复了这个问题，现在脚本会确保在正确的目录中操作文件。如果您仍然遇到这个问题，可以尝试手动检查文件：
...
### **🔟 为什么account.json文件格式不正确？**

如果您看到`account.json`文件中有格式错误，这可能是由于之前的版本导致的。我们已经修复了这个问题，现在`account.json`文件是纯文本格式，每行一个私钥。如果您遇到格式问题，可以尝试重新运行脚本，或者手动修复文件格式。
