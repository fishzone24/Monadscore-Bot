...
这可能是由于文件路径问题导致的。我们已经修复了这个问题，现在脚本会确保在正确的目录中操作文件，并使用Python正确处理JSON格式。如果您仍然遇到这个问题，可以尝试手动检查文件：
...
如果您看到`account.json`文件中有格式错误（如多余的逗号），这可能是由于之前的版本导致的。我们已经修复了这个问题，现在`account.json`文件只包含私钥列表，格式为`["私钥1", "私钥2", ...]`。如果您遇到格式问题，可以尝试重新运行脚本，或者手动修复文件格式。
